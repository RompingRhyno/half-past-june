...
import { SubmitResult } from "@/components/product-form/ProductForm";
...
export async function updateProduct(data: UpdateProductInput) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const incomingVariantIds = incomingVariants.map((v) => v.id).filter(Boolean) as string[];
      ...
      if (variantIdsToDelete.length > 0) {
        await tx.variantSize.deleteMany({ where: { variantId: { in: variantIdsToDelete } } });
      ...
      for (const variant of incomingVariants) {
        if (variant.id) {
          ...
          const existingSizes = existingVariants.find((v) => v.id === variant.id)?.sizes || [];
          ...
          const sizeIdsToDelete = existingSizeIds.filter((id) => !incomingSizeIds.includes(id));
          if (sizeIdsToDelete.length > 0) {
            await tx.variantSize.deleteMany({ where: { id: { in: sizeIdsToDelete } } });
          }
          ...
          for (const size of incomingSizes) {
            if (size.id) {
              await tx.variantSize.update({
                where: { id: size.id },
                data: { size: size.size, stock: makeToOrder ? null : Number(size.stock || 0) },
              });
            } else {
              await tx.variantSize.create({
                data: { variantId: variant.id, size: size.size, stock: makeToOrder ? null : Number(size.stock || 0) },
              });
            }
          }
        } else {
          ...
          await tx.productVariant.create({
            data: {
              ...
              price: Number(variant.price),
              stock: variant.sizes?.length ? null : Number(variant.stock || 0),
              sizes: { create: variant.sizes?.map((s) => ({ size: s.size, stock: makeToOrder ? null : Number(s.stock || 0) })) || [] },
            },
          });
        }
      }
...
export async function updateProductAction(formData: FormData): Promise<SubmitResult> {
  try {
    await updateProduct({
